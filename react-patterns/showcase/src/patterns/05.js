import React, {Component, useCallback, useLayoutEffect ,createContext, useState,
    useMemo, useContext, useEffect, useRef } from 'react';
...
const useClapAnimation = ({ clapEl,
   ...
   useLayoutEffect(()=>{
     ...
     const newAnimationTimeline = animationTimeline.add([scaleButton, countTotalAnimation, 
       countanimation, triangleBurst, circleBurst]);
...
return (
   <Provider  value={memoizedValue}>
       <button ref={setRef} data-refkey="clapRef" className={classNames} 
...
const classNames = [styles.count, className].join(' ').trim()
 return <span ref={setRef} data-refkey="clapCountRef" className={classNames}
    style={userStyles}>
   + {count}
 </span>
}
...
const CountTotal = ({style: userStyles ={}, className})=>{
 ...
 return <span ref={setRef} data-refkey="clapTotalRef" className={classNames}
    style={userStyles}>
   {countTotal}
 </span>;
}
...
const Usage = () =>{
   ...
   const handleClap = () =>{
       setState(({count, countTotal})=>({
        ...
        countTotal: count < count < 10 ? countTotal + 1 : countTotal,
        isClicked: true
       }))
   }
 return (
   <div style ={{ width: '100%'}}>
<MediumClap onClap = {handleClap} values={state} className={userCustomStyles.clap}>
